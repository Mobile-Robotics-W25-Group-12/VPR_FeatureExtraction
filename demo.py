...
def main():
    ...
    if args.descriptor == "HDC-DELF":
        ...
        if args.descriptor == "NetVLAD":
            configfile = os.path.join(
                PATCHNETVLAD_ROOT_DIR, "configs/netvlad_extract.ini"
            )
        else:
    ...
    if args.descriptor != "PatchNetVLAD" and args.descriptor != "SAD":
        ...
        db_D_holistic = db_D_holistic / np.linalg.norm(
            db_D_holistic, axis=1, keepdims=True
        )
        q_D_holistic = q_D_holistic / np.linalg.norm(
            q_D_holistic, axis=1, keepdims=True
        )
        ...
        S = feature_extractor.local_matcher_from_numpy_single_scale(
            q_D_patches, db_D_patches
        )
